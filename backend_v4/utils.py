...
def find_code_files_util(directory):
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for file in files:
            ...
            file.endswith('.c') or file.endswith('.rb') or file.endswith('.r') or file.endswith('.ts')):
...
def normalize_scores(scores):
    ...
    # Check for the edge case where total_sum is 0 to avoid division by zero
    if total_sum == 0:
        return {key: 0 for key in scores}

    # Calculate percentage for each score and update the dictionary
    normalized_scores = {key: (value / total_sum) * 100 for key, value in scores.items()}
